...
void SetPixel(int x, int y, unsigned int color, bool clean)
{
...
void DrawLine(short x1, short y1, short x2, short y2, unsigned int color, bool clean)
{
  short dx = abs(x2 - x1);
  short sx = x1 < x2 ? 1 : -1;
...
{
  DrawLine(x0, y0, x1, y1, color, clean);
  DrawLine(x1, y1, x2, y2, color, clean);
  DrawLine(x2, y2, x0, y0, color, clean);
}
